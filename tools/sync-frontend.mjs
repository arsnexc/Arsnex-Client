...
const BRIDGE = [
  ...
  '',
  '  // Instance creation progress, emitted by the real download pipeline.',
  "  listen('instance://stage', e=>{",
  '    const s=e.payload;',
  '    if(window.__onInstanceStage)window.__onInstanceStage(s);',
  "    CON.push('INFO','Instance', s.label + (s.detail ? ' — ' + s.detail : ''));",
  '  });',
  '',
  "  window.__createInstance = (req)=>invoke('create_instance',{req});",
  "  window.__listInstances  = ()=>invoke('list_instances');",
  "  window.__deleteInstance = (slug)=>invoke('delete_instance',{slug});",
  "  window.__checkName      = (name)=>invoke('check_instance_name',{name});",
  '',
